...
import { Component, OnChanges, SimpleChanges, computed, input } from '@angular/core';
...
export class CalendarComponent implements OnChanges {
  ...
  dayLabels = computed(() => { return this.calendar.dayLabels});
  monthLabels = computed(() => { return this.calendar.monthLabels});
  monthData = computed(() => { return this.calendar.getMonthData(this.year())});
  weekFirstDay = computed(() => { return this.calendar.weekFirstDay});
  cellOffset = computed(()=> { return this.monthData()[0].firstDayWeekIndex - this.weekFirstDay();})
  cellCount = computed(() => { return this.calendar.isLeapYear(this.year()) ? 366 : 365});
